...
// Definición de la interfaz de la operación
REGISTER_OP("ZeroOut")
    .Input("to_zero: int32")
    .Output("zeroed: int32")
...
class ZeroOutOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    // Crear un output tensor con la misma forma
    Tensor* output_tensor = NULL;
    OP_REQUIRES_OK(context, context->allocate_output(0, input_tensor.shape(),
                                                     &output_tensor));
    auto output_flat = output_tensor->flat<int32>();

    // Poner la primera posición tal cual y el resto a 0 (ejemplo básico)
    const int N = input.size();
    for (int i = 1; i < N; i++) {
      output_flat(i) = 0;
    }

    // Preservar el primer elemento si existe
